...
func (self *PCNNetwork) TrainMap(inputs []map[string]float64, targets [][]float64, iteration int) {
	...
	for i := 0; i < iteration; i++ {
		...
		if (iteration >= 10 && (i+1)%(iteration/10) == 0) || iteration < 10 {
			fmt.Printf("\niteration %v err: %v", i+1, cur_err / float64(len(inputs)))
		}
	}
	fmt.Println("\ndone.")
}
...
func (self *PCNNetwork) ForwardMapForPredicate(input map[string]float64) []float64{
	...
	for i := 0; i < len(self.OutputLayer); i++ {
		...
		for j,_:= range input {
			...
			sum += input[j] * self.WeightOutput[i][j]
		}
		if(self.Regression){
			self.OutputLayer[i] = sum
		}else{
